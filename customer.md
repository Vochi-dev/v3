...
## To‑Do: Автоматическое обновление ФИО из RetailCRM
**Цель**: При любом входящем звонке (независимо от УП) получать ФИО клиента из RetailCRM и обновлять в `customers`.

1) **Модификация логики обработки звонков**:
   - В `app/services/calls/hangup.py` добавить проверку: если RetailCRM активен (не обязательно главный), получить профиль клиента
   - Извлечь ФИО из ответа RetailCRM (`source.raw`)
   - Вызвать `merge_customer_identity` с `source='retailcrm'` и полученными данными
   - При совпадении номера автоматически обновлять ФИО в таблице `customers`

2) **Реализация в dial.py** (опционально):
   - Аналогично для dial события для быстрого обновления данных при начале разговора
   - Приоритет на hangup событие как основной источник истины

3) **Настройка приоритета источников**:
   - RetailCRM как источник ФИО работает независимо от настройки "главная интеграция"
   - При конфликте с другими источниками использовать timestamp последнего обновления

4) **Unit‑тесты**:
   - Создание клиента в RetailCRM с определенным ФИО
   - Входящий звонок от этого номера → проверка обновления ФИО в `customers`
   - Изменение ФИО в RetailCRM → новый звонок → проверка обновления
   - Проверка `meta.ids.retailcrm` и `meta.person_uid`

### Статус реализации:
- [x] Модификация `hangup.py` для интеграции с RetailCRM **ТОЛЬКО для приоритетной интеграции**
- [x] Добавление извлечения ФИО из `source.raw` ответа RetailCRM  
- [x] Создание unit-тестов (`test_retail_fio_updates.py`)
- [x] Реализация функционала множественных номеров через `person_uid`
- [x] Тестирование обновления ФИО для всех номеров клиента
- [x] Документирование результатов тестов в customer.md

### Тест‑кейсы и результаты:
- **✅ Тест 1**: Создание записи с `person_uid` и `meta.ids.retailcrm` 
  - Результат: Запись создана с `person_uid: "retailcrm:12345"`, `meta.ids.retailcrm: ["12345"]`
- **✅ Тест 2**: Связывание второго номера с тем же `person_uid`
  - Результат: Оба номера (+375296254070, +375296111333) связаны через `person_uid: "retailcrm:12345"`
- **✅ Тест 3**: Обновление ФИО для всех номеров клиента
  - Результат: ФИО обновилось для обоих номеров одновременно (Александр Иванов Петрович)
- **✅ Тест 4**: Проверка приоритетной интеграции
  - Результат: Система обновляет ФИО только для `primary_integration = "retailcrm"`

### Ключевые улучшения:
1. **Приоритет интеграций**: ФИО обновляется только для приоритетной интеграции (`primary_integration`)
2. **Множественные номера**: Клиент может иметь несколько номеров, связанных через `person_uid`
3. **Автоматическая синхронизация**: Изменение ФИО автоматически применяется ко всем номерам клиента
4. **Безопасность**: Не приоритетные интеграции только записывают `external_id`, но не перезаписывают ФИО
