...
from app.services.email_verification import (
    verify_token_and_register_user,
)
...
async def verify_email(request: Request, token: str | None = None):
    ...
    try:
        # Попытка подтвердить токен и зарегистрировать пользователя
        await verify_token_and_register_user(token)
        logger.info(f"Email подтверждён, токен={token}")
    except RuntimeError as e:
        # непрошёл проверку (не нашли, просрочен и т.п.)
    ...
    # всё прошло — перенаправляем или показываем успех
    return templates.TemplateResponse(
        "verify_result.html",
        {
            "request": request,
