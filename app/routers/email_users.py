...
async def upload_email_users(request: Request, file: UploadFile = File(...)):
    ...
    # без удалений — сразу перезаписываем таблицу
    db = await get_connection()
    try:
...
async def confirm_upload(
    ...
    # удаляем и отправляем уведомления
    db = await get_connection()
    try:
        ...
        for row in rows:
            ...
            if email and email not in new_set and row["tg_id"] and row["bot_token"]:
                # уведомляем
                ...
                # удаляем все следы
                await db.execute("DELETE FROM telegram_users    WHERE tg_id = ?", (row["tg_id"],))
                await db.execute("DELETE FROM enterprise_users WHERE telegram_id = ?", (row["tg_id"],))
                await db.execute("DELETE FROM email_users       WHERE email = ?", (email,))
        # после этого записываем новый CSV
        await db.execute("DELETE FROM email_users")
        reader = csv.DictReader(io.StringIO(text))
        for row in reader:
            await db.execute(
...
async def delete_user(tg_id: int, request: Request):
    ...
    # 3) наконец удаляем все следы этого пользователя
    db = await get_connection()
    try:
        ...
        await db.execute("DELETE FROM email_users       WHERE email IN (SELECT email FROM telegram_users WHERE tg_id = ?)", (tg_id,))
        await db.commit()
