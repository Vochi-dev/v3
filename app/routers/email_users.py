...
# загрузка CSV и синхронизация остаются без изменений…
# (…код upload_email_users и confirm_upload…)
...
async def delete_user(tg_id: int, request: Request):
    ...
    # 1. Пытаемся получить bot_token из telegram_users
    bot_token = None
    ...
    # 3. Отправляем **единственное** фиксированное сообщение
    if bot_token:
        try:
            ...
            if sent:
                logger.info(f"[delete] Сообщение отозван доступ отправлено {tg_id}")
            else:
                logger.warning(f"[delete] Не удалось отправить сообщение {tg_id}")
        except Exception as e:
            logger.error(f"[delete] Ошибка при отправке сообщения {tg_id}: {e}")
    else:
        logger.error(f"[delete] bot_token для {tg_id} не найден — сообщение не отправлено")

    # Временно **не удаляем** ничего из базы — только сообщение
