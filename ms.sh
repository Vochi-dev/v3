...
# =============================================================================
# МойСклад Integration Service Controller
# =============================================================================
...
stop() {
    ...
    # Дополнительный поиск процессов ms по имени
    MANUAL_PIDS=$(pgrep -f "python.*ms\.py" || true)
    if [ -n "$MANUAL_PIDS" ]; then
        echo "🔍 Найдены вручную запущенные процессы ms: $MANUAL_PIDS"
        FOUND_PIDS="$FOUND_PIDS $MANUAL_PIDS"
    fi

    # Поиск uvicorn процессов на порту 8023
    UVICORN_PIDS=$(pgrep -f "uvicorn.*ms.*8023" || true)
    if [ -n "$UVICORN_PIDS" ]; then
        echo "🔍 Найдены uvicorn процессы на порту 8023: $UVICORN_PIDS"
...
test() {
    echo "🧪 Тестирование МойСклад API..."
    ...
    echo ""
    echo "🔸 2. Тест подключения к МойСклад..."
    curl -s "http://localhost:$PORT/test/credentials" | python3 -m json.tool

    echo ""
    echo "🔸 3. Тест получения организаций..."
    curl -s "http://localhost:$PORT/test/organizations" | python3 -m json.tool
...
case "$1" in
    ...
    *)
        ...
        echo ""
        echo "Команды:"
        echo "  start   - Запустить сервис МойСклад"
