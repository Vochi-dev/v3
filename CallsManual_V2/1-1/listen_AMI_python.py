...
def NewCallerid(manager, message):
    ...
    # ✅ МОДЕРНИЗАЦИЯ: Отправляем NewCallerid в альтернативный API
    if apiAlternative == 1:
        apiAlternativeQueue.put(amiEvent)
...
def BridgeCreate(manager, message):
    ...
    # ✅ МОДЕРНИЗАЦИЯ: Отправляем BridgeCreate в альтернативный API
    if apiAlternative == 1:
        apiAlternativeQueue.put(amiEvent)
...
def BridgeLeave(manager, message):
    ...
    # ✅ МОДЕРНИЗАЦИЯ: Отправляем BridgeLeave в альтернативный API
    if apiAlternative == 1:
        apiAlternativeQueue.put(amiEvent)
...
def BridgeDestroy(manager, message):
    ...
    # ✅ МОДЕРНИЗАЦИЯ: Отправляем BridgeDestroy в альтернативный API
    if apiAlternative == 1:
        apiAlternativeQueue.put(amiEvent)
...
def Alternative_crm_post(data: AMIEvent):
    ...
    if data.Event == 'localcallStart':  
        ...
        # Если Phone или Extensions пустые - ищем данные в AMI
        if not data.Phone or (hasattr(data, "Extensions") and isinstance(data.Extensions, list) and len(data.Extensions) == 1 and data.Extensions[0] == ""):
            # Ищем DialBegin событие для этого UniqueId
            from datetime import datetime
            import sqlite3
            db_file = "/var/log/asterisk/Listen_AMI_" + datetime.now().strftime("%Y-%m-%d") + ".db"
            try:
                conn = sqlite3.connect(db_file)
                cursor = conn.cursor()
                cursor.execute("SELECT CallerIDNum, Exten FROM AMIEventLog WHERE Event=? AND UniqueId=? ORDER BY id DESC LIMIT 1", ("DialBegin", data.Uniqueid))
                result = cursor.fetchone()
                if result:
                    ami_caller, ami_exten = result
                    if not data.Phone:
                        data.Phone = ami_caller
                    if hasattr(data, "Extensions") and isinstance(data.Extensions, list) and len(data.Extensions) == 1 and data.Extensions[0] == "":
                        data.Extensions = [ami_exten]
                conn.close()
