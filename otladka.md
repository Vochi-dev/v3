...
- ✅ Исправлено форматирование Telegram сообщений для dial: 
  - Добавлен заголовок "📞 Исходящий звонок" для `CallType == 1`
  - Менеджер: "ФИО (номер)" или просто "номер" (без "Доб.")
- ✅ Исправлена проблема с кэшем 8020: добавлена "ленивая загрузка" в `is_line_exists` и `is_manager_exists`

### 2025-10-01 - Обогащение события bridge
- ✅ **Файл:** `app/services/calls/bridge.py`
- ✅ Исправлена та же ошибка с `enterprise_number = token[:4]` → теперь запрос к БД
- ✅ Включено обогащение метаданными для bridge событий (было отключено)
- ✅ Добавлен заголовок для исходящих: "🔗 Идет исходящий разговор"
- ✅ Формат менеджера: "ФИО (номер)" если есть ФИО, иначе просто "номер"
- ✅ Формат клиента: "ФИО номер" если есть в БД, иначе просто "номер"
- ✅ Добавлено отображение линии с обогащением: "Название (trunk)" или просто "trunk"
- ✅ **ИСПРАВЛЕНО:** Извлечение `Trunk` из поля `Channel` (например: `"SIP/0001363-00000001"` → `"0001363"`)
  - В событии bridge поле `Trunk` отсутствует, поэтому парсим `Channel`
- ✅ **КРИТИЧЕСКАЯ ОШИБКА:** Неправильный параметр в вызове `enrich_message_data()`
  - Было: `manager_internal_number` → Стало: `internal_phone`
  - Также исправлено: `customer_phone` → `external_phone`
- ✅ **Единообразие форматирования bridge:**
  - Клиент: `+375 (29) 625-40-70 (Тестовый покупатель)` - сначала номер, потом ФИО в скобках
  - Линия: `📡 А1 1` - антенна 📡 + название (без номера линии в скобках)
- ✅ **КРИТИЧЕСКАЯ ОШИБКА:** Неправильное определение `call_direction` для bridge
  - В bridge для исходящих: `CallerIDNum`=внешний, `ConnectedLineNum`=внутренний (роли перевернуты!)
  - Исправлена логика: если `!caller_internal && connected_internal` → это `outgoing`
  - Теперь заголовок "🔗 Идет исходящий разговор" отображается корректно
- ✅ **КРИТИЧЕСКАЯ ОШИБКА:** Дублирование bridge сообщений (отправлялись 4 раза)
  - Проблема: `_dispatch_to_all` вызывает `process_bridge` для каждого чата, а `process_bridge` вызывал `send_bridge_to_telegram`, которая снова перебирала все чаты
  - Исправлено: `process_bridge` теперь вызывает `send_bridge_to_single_chat` напрямую
- ✅ **Удалены все .pyc файлы и __pycache__** для применения изменений
- ✅ **НАЙДЕНА КОРНЕВАЯ ПРИЧИНА "Доб.152 (152)":**
  - Сервис 8020 (metadata_client) возвращает `'manager_name': 'Доб.152'` для менеджеров без ФИО
  - В bridge.py добавлена проверка: `if manager_fio and not manager_fio.startswith("Доб.")` 
  - Теперь если `manager_name` = "Доб.152", показывается просто "152"
- 🔄 Ожидается перезапуск сервисов и повторное тестирование шагов 1-4
- ⚠️ **ОБНАРУЖЕНА ПРОБЛЕМА:** События `bridge_create`, `bridge_leave`, `bridge_destroy` отправляются в Telegram (дубликаты), хотя не должны по логике

## 💾 Принципы работы кэша событий

### 📋 Структура кэша в RAM
Все промежуточные события (до hangup) хранятся в оперативной памяти сервера:

```python
# Основные хранилища (индивидуальные для каждого chat_id)
phone_message_tracker_by_chat = {
    chat_id: {
        phone: {
            'message_id': 12345,           # ID сообщения в Telegram
            'event_type': 'bridge',        # тип события (dial/bridge)
            'timestamp': '2025-10-02T07:23:54',
            'unique_id': '1759389248.0',
            'call_type': 1
        }
    }
}
bridge_store_by_chat = {chat_id: {unique_id: message_id}}
dial_cache_by_chat = {chat_id: {unique_id: dial_data}}
```

### 🔗 Связывание событий
**Ключевой принцип:** События связываются по **внешнему номеру телефона**, а НЕ по UniqueId!

- **Dial:** `Phone: "375296254070"` → группируется по `"375296254070"`
- **Bridge:** `CallerIDNum: "375296254070"` → группируется по `"375296254070"` ✅ **СОВПАДЕНИЕ**
- **Hangup:** `Phone: "375296254070"` → ищет bridge по `"375296254070"` ✅ **НАЙДЕТ**

### ⚡ Жизненный цикл события
1. **Dial приходит** → сохраняется в `phone_message_tracker_by_chat[chat_id]["375296254070"]`
2. **Bridge приходит** → находит dial в кэше → удаляет dial из Telegram → заменяет в кэше
3. **Hangup приходит** → находит bridge в кэше → удаляет bridge из Telegram → очищает кэш

### 🧹 Автоочистка кэша
- **Триггер:** При каждом новом событии (НЕ по расписанию!)
- **Лимит:** Максимум 1,000 записей на chat_id
- **Алгоритм:** При превышении лимита удаляются 100 самых старых записей
- **Потребление RAM:** ~316 КБ максимум (не зависит от общего количества разговоров)

### ⏰ Временные ограничения
- **10-минутное ограничение** действует только для комментариев (`should_send_as_comment`)
- **Замена сообщений** (`should_replace_previous_message`) НЕ имеет временных ограничений
- **Hangup удалит bridge** даже после 2-часового разговора!

### 🔄 Поведение при перезапуске
- **Перезапуск сервера** → весь кэш обнуляется
- **Hangup после перезапуска** → НЕ найдет bridge для удаления (останется "висеть" в Telegram)
